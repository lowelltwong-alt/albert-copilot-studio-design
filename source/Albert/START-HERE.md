...
**Other versions:** [No MCP or API](No%20MCP%20or%20API/START-HERE.md) · [MCP & API](MCP%20%26%20API/START-HERE.md).

[More](More/README.md) holds setup details, templates, examples, reviews, tests, credits and supporting design material. The separately sendable `Albert-Digital-Assets-1.0.zip` contains the operational local MCP/API asset service and its AI front door. The example remains held with documented improvements; Studio tenant testing is pending.
