...
Use this new synthetic case to learn the setup. The target is a useful short exercise, not a gold comparison or a 30-minute case product. Keep [SOURCE](demo/SOURCE.md) and [OPERATOR-BRIEF](demo/OPERATOR-BRIEF.md) unchanged while comparing attempts. The [worked example and actual review](demo/00-READ-THIS-EXAMPLE.md) and instructor-only probes are comparison aids, not inputs for a fresh Writer.

## Prepare once

Create an ordinary approved folder for this practice and copy the source, operator brief and [STATE](templates/STATE.md) template into it. Use `LANTERN-MINA` as the prefix and `R01` as the initial assembly revision. Open the four unpublished agents from [setup](01-COPILOT-SETUP.md). Record the actual model labels and context in the [configuration worksheet](templates/CONFIGURATION.md).

Paste the named complete RUN file into the stated role's Test agent chat, then paste the required content in a [PACKET](templates/PACKET.md). Send **BEGIN RUN** only when all required content has arrived. If the full RUN is rejected, use the ordered smaller paste files listed in [placement](07-PROMPT-PLACEMENT.md). Do not substitute a filename for file content.
