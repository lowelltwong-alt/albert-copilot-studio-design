...
Required inputs: SOURCE, operator scope/permissions, case/witness/use, GRAPH template, state packet

Saved output suffixes: W05-GRAPH, W05-VIEW, W05-AUTHOR, STATE
...
Build the graph using the supplied GRAPH table schema and the source's own stable anchors. Inventory documents and semantic exhibits/attachments separately. Extract attributed claims, negations, perception/later-learning lanes, both conflict endpoints and unknowns. Record explicit source/permission paths, not merely file citations. Only operator-approved grants may enter VIEW; proposed grants remain proposed. Keep excluded/author-only content in W05-AUTHOR, never in VIEW, including hints or metadata. Run TRACE on one important claim and VIEW for the selected witness/use/time. Return all input and graph gaps. The operator and Evidence role inspect graph/view with RUN-03 before drafting. Do not create the plan or script yet.
